...
* **Space Complexity:** The algorithm requires memory to store the sequences O(N * L), the current set of motifs O(N * k), the PWM O(4 * k), and the background model. The overall space complexity is tightly bounded by the input size: O(N * L).
